...
class TempControl:
	...
	def setReferenceTemperature(self, t_ref):				# SET THE REFERENCE TEMPERATURE
		...
		if (t_ref == None):																				# non dovrebbe essere "Null"?
			self.t_ref = None
		else:
			self.t_ref = float(t_ref)

	def setCurrentLight(self, light):
		
		self.light = str(light)

	def temp_control(self, temp):									# TEMPERATURE CONTROL

		if (self.t_ref == None or self.light == None):		# If control is not activated or if you don't know if the light is on or off
			return None
